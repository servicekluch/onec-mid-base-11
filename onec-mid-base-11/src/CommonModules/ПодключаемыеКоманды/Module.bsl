...
Процедура ПриСозданииНаСервере(Форма, Знач ПараметрыРазмещения = Неопределено) Экспорт
	...
	//++Егорихин М.Ю. 17.06.2024 Программное добавленгие элементов и команд на форму++
	Элементы = Форма.ЭтотОбъект.Элементы;
	Команды = Форма.ЭтотОбъект.Команды;
	Если СокрЛП(Форма.ЭтотОбъект.ИмяФормы) = "Документ.ЗаказПокупателя.Форма.ФормаДокумента" Тогда

		ПолеФормы = Элементы.Добавить("КонтактныеЛица",Тип("ПолеФормы"),Элементы.ГруппаШапкаПраво);
		ПолеФормы.Заголовок = "Контактное лицо";
		ПолеФормы.Вид = ВидПоляФормы.ПолеВвода;
		ПолеФормы.ПутьКДанным = "Объект.КИ_КонтактноеЛицо";
		
		КомандаФормы = Команды.Добавить("ПересчетСкидки");
		КомандаФормы.Заголовок = "Пересчет скидки";
		КомандаФормы.Действие = "КИ_ПересчетСкидки";
		
		ГруппаФормы = Элементы.Добавить("Группа1", Тип("ГруппаФормы"), Элементы.ГруппаШапкаЛево);
		...
		ПолеФормы = Элементы.Добавить("СогласованнаяСкидка",Тип("ПолеФормы"),ГруппаФормы);
		ПолеФормы.Заголовок = "Согласованная скидка";
		ПолеФормы.Вид = ВидПоляФормы.ПолеВвода;
		ПолеФормы.ПутьКДанным = "Объект.КИ_СогласованнаяСкидка";
		ПолеФормы.УстановитьДействие("ПриИзменении", "СогласованнаяСкидкаПриИзменении");

		КнопкаФормы = Элементы.Добавить("ПересчетСкидки",Тип("КнопкаФормы"),ГруппаФормы);
		...
		КнопкаФормы.Фигура = ФигураКнопки.Обычная;
		КнопкаФормы.ОтображениеФигуры = ОтображениеФигурыКнопки.ПриАктивности;
	ИначеЕсли СокрЛП(Форма.ЭтотОбъект.ИмяФормы) = "Документ.ОплатаОтПокупателя.Форма.ФормаДокумента"
		ИЛИ СокрЛП(Форма.ЭтотОбъект.ИмяФормы) = "Документ.ОплатаПоставщику.Форма.ФормаДокумента" Тогда 
		
		ПолеФормы = Элементы.Добавить("КонтактныеЛица",Тип("ПолеФормы"));
		ПолеФормы.Заголовок = "Контактное лицо";
		ПолеФормы.Вид = ВидПоляФормы.ПолеВвода;
		ПолеФормы.ПутьКДанным = "Объект.КИ_КонтактноеЛицо";
	ИначеЕсли СокрЛП(Форма.ЭтотОбъект.ИмяФормы) = "Документ.ПоступлениеТоваровУслуг.Форма.ФормаДокумента"
		ИЛИ СокрЛП(Форма.ЭтотОбъект.ИмяФормы) = "Документ.РеализацияТоваровУслуг.Форма.ФормаДокумента"Тогда 
